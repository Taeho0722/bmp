...
if (!String.prototype.endsWith) {
    String.prototype.endsWith = function (strToSearch) {
        var strSearchLength = strToSearch.length;
        if (!this || !strToSearch) return false;
        if (this.substring(this.length - strSearchLength, this.length) === strToSearch) {
            return true;
        } else return false;
    }
}
if (!String.prototype.startsWith) {
    String.prototype.startsWith = function (strToSearch) {
        return strToSearch.length > 0 && this.substring(0, strToSearch.length) === strToSearch;
    }
}
//문자열 파싱
...
ozf_stringbuilder.prototype.replace = function (fromstr, tostr) {
    for (var i = this.buffer.length - 1; i >= 0; i--) {
        this.buffer[i] = this.buffer[i].replace(new RegExp(fromstr, "g"), tostr);
    }
}
//문자열로 반환한다.
...
function ozf_getSelectedNodes(xPnode, path) {
    ...
    xfind = ozf_findNodeByTagName(xPnode.childNodes, list[0]);
...
function gRefMain(win) {
    // var frameEle; 
    // frameEle = $(win.parent.document).find("object")[0];
    // if(frameEle == undefined) frameEle = $(win.parent.document).find("iframe")[0];

    // var aDiv = frameEle.parentElement;
    // if (aDiv.className.indexOf("popup") >= 0) {
    //     //'Pop window
    //     var pParent = win.parent.parent;
    //     while ($(pParent.parent.document).find("object")[0] != undefined) {
    ...
    if($(win.parent.document).find("object").length > 0) {
        var frameEle = ozf_Rootbodyframe(win.parent);
    } else {
        var frameEle = ozf_Rootbodyframe(win);
    }
...
function ozf_OpenDialogOpenPage(Sender, PageCaption, HtmlName, w, h, tag, notdefinesize) {
    ...
    if (pObj.document.defaultView.register_DialogInst != undefined) {
        pObj.document.defaultView.register_DialogInst(HtmlName.replace('.html', ''), window);
    }
    else {
        if (pObj.register_DialogInst != undefined) {
            pObj.register_DialogInst(HtmlName.replace('.html', ''), window);
        }
    }    
    p.show();
}
...
Return :  서버 날짜
----------------------------------------------------*/
function ozf_GetServerDate(delim) {
...
Return :  서버 시간(시, 분, 초)
----------------------------------------------------*/
function ozf_GetServerDateTimeHMS() {
    var metah = new oza_MetaHandler("ServerHelper", "<List><Item address=''/></List>", "<List><Info option='servertime'/></List>")
    metah.execute(null, false);
...
function ozf_GetServerDateTime() {
    var metah = new oza_MetaHandler("ServerHelper", "<List><Item address=''/></List>", "<List><Info option='servertime'/></List>")
    metah.execute(null, false);
    ...
    return serverdate.replace(/-/g, '').replace(/:/g, '').replace(/\s/g, '');
}
...
function ozf_ConvertToYYYY_MM_DD(value, delim) {
    if (delim == null) { delim = "-"; }
    if (value.length == 8) {
        return "{0}{3}{1}{3}{2}".format(value.substr(0, 4), value.substr(4, 2), value.substr(6, 2), delim);
    }
    return value;
}
...
function ozf_ConvertDateToString(varDate, delim) {
    ...
    return "{0}{3}{1}{3}{2}".format(yyyy, mm, dd, delim);
}
 ...
 * @param string
 */
function ozf_Decode(str) {
     var ret;
    ret = ozf_AgentHelperHandler("AgentHelper", "DecodeString", str)
    return ret;
    //var decStr;
    //var head = document.getElementById('head');
    //var oldCharsetTag;
...
function ozf_MetaTranslate(addr) {
    ...
    var errormsg;
    var metahandler1 = new oza_MetaHandler("ServerHelper", addr, "<List><Item option='translate'/></List>", true);
    metahandler1.execute();
    if (oza_CheckMetaReturn(metahandler1.returncontent) == true) {
        ...
        ozf_MsgBox(oza_GetErrorContent(metahandler1.returncontent));
        return "";
    }
}
...
function ozf_JSONStringToString(JSONStr, Delimiter) {
    ...
    if (jsonObj.length > 0) {
        keylist = Object.keys(jsonObj[0]);
        for (var i = 0 ; i < jsonObj.length; i++) {
            item = jsonObj[i];
            for (var j = 0; j < keylist.length; j++) {
                data.push(item[keylist[j]]);
            }
        }
...
//Collection에 삭제
if (!ozf_Collection.prototype.remove) {
    ozf_Collection.prototype.remove = function (sKey, oValue) {
        var r = -1;
        if (sKey == null) return false;
        if (sKey === "") return false;
        for (var i = 0, keyslen = this.Keys.length; i < keyslen; i++) {
...
function ozf_geticonpath(itemtype) {
    switch (itemtype.toLowerCase()) {
        case "managebaseserver":
            return "../img/type/managebaseserver.png";
        case "privatebaseserver":
            return "../img/type/privatebaseserver.png";
        case "publicbaseserver":
            return "../img/type/publicbaseserver.png";
        case "cube":
            return "../img/type/cube.png";
        case "cubefolder":
            return "../img/type/cubefolder.png";
        case "reports":
        case "analyticreports":
            return "../img/type/reports.png";
        case "report":
        case "analyticreport":
            return "../img/type/report.png";
        case "crossreport":
            return "../img/type/crossreport.png";
        case "listreport":
            return "../img/type/listreport.png";
        case "dashboard":
            return "../img/type/dashboard.png";
        case "folder":
            return "../img/type/folder.png";
        case "dimensions":
            return "../img/type/dimensions.png";
        case "dimension":
            return "../img/type/dimension.png";
        case "facts":
            return "../img/type/facts.png";
        case "fact":
            return "../img/type/fact.png";
        case "measures":
            return "../img/type/measures.png";
        case "measure":
            return "../img/type/measure.png";
        default:
            return ""
    }
}
...
function ozf_GetType(obj, typename) {
    ...
    if(obj.constructor.name == undefined) {
        cont = obj.constructor.toString();
    }
    else {
        cont = obj.constructor.name;
    }

    if(cont.indexOf(typename) > -1) {
        return true;
    }
    else {
        if (isallowbutton) {
            if (cont.toLowerCase().indexOf("oz_button") > -1) {
                return true;
            }
        }
        return false;
...
function ozf_isNumeric(val) {
    return $.isNumeric(val);
}

function ozf_isIE() {
    if (navigator.userAgent.search("MSIE") >= 0 || navigator.userAgent.search("Trident") >= 0 || navigator.userAgent.search("Edge") >= 0) return true;
    else return false;
}