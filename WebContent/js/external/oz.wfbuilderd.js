 ...
 * ToDo>>>
 * Diagram 종류별 UI 구현, 차이점 비교 : D3FBuilder Only
 *      event : executeactivity/executeallactivity/visualization
 * //D3FBuilder Only
 * -- DoubleClick시 EvtArgs.Purpose값이 "OpenDialog" 가 아니라 SelAct.ActName
 * -- validCycle(initDiagram), BeforeActivityDelete(cxcommand2, EvtArgs.Purpose)
 * -- ??? ExportImage 기능???
...
//=== ozbuilderd==========B=======================================================//
function ozbuilderd(sID) { //parentDivID, contextMenuDivID) {
    this.mvarid = '#' + sID;
    this.ctl;
    ...
    refPDiv.innerHTML = "<div id='divBuilder1' class='oz-wf-Div-BuilderD'></div>" +
        "<div id='divWfCM1' class='oz-wf-cm-d'><ul></ul></div>";
    ...
    this.MPAppID;
    this.IPAppID;
    this.LayoutDirection = 1;   //0:Horizon 1:Vertical
    this.ZoomPercent = 100;
    this.InnerDockPos = 3;
    this.Editable = false; //this version
    this.RunAsMonitor = false;
    this.RaiseEvent_BeforeStateChange = false;
    this.ManualActivityDelete = true;
    //활성화 여부 설정
    ...
    this.executeactivity = function (e) { }
    this.executeallactivity = function (e) { }
    this.visualization = function (e) { }
    //Event_End==================
...
if (!ozbuilderd.prototype.addEventListener) {
    ozbuilderd.prototype.addEventListener = function (e, f) {
        ...
        } else if (e == "executeactivity") {    //D3FBuilder Only
            this.executeactivity = f;
        } else if (e == "executeallactivity") {
            this.executeallactivity = f;
        } else if (e == "visualization") {
            this.visualization = f;
        }
    }
}
if (!ozbuilderd.prototype.dispose) {
    ozbuilderd.prototype.dispose = function () {
...
//활성화 여부 설정
if (!ozbuilderd.prototype.getEnabled) {
    ozbuilderd.prototype.getEnabled = function () {
        return !$(this.mvarid).prop('disabled');
    }
}
if (!ozbuilderd.prototype.setEnabled) {
    ozbuilderd.prototype.setEnabled = function (val) {
        $(this.mvarid).prop('disabled', !val);
    }
}
//Visible 여부
if (!ozbuilderd.prototype.getVisible) {
    ozbuilderd.prototype.getVisible = function () {
...
if (!ozbuilderd.prototype.setVisible) {
    ozbuilderd.prototype.setVisible = function (val) {
        if (val == true) {
...
//Puplic API===================
if (!ozbuilderd.prototype.GetData) {
    ozbuilderd.prototype.GetData = function (sPurpose, sAddress, sContent) {
...
if (!ozbuilderd.prototype.SetData) {
    ozbuilderd.prototype.SetData = function (sPurpose, sAddress, sContent) {
        sPurpose = sPurpose.toLowerCase();
        switch (sPurpose) {
            case "dialogresult":
                //D3FBuilder Only
                ...
                var sFromID = ozw_GetAttrValue(xDoc.documentElement.childNodes[0], "fromactivity", "");
                var pSelWfAct = gProcActs.Item(sFromID);
...
if (!ozbuilderd.prototype.recDelActsLinkedFrom) {
    ozbuilderd.prototype.recDelActsLinkedFrom = function (pWfActSrc, bDoRecursive) {
...
if (!ozbuilderd.prototype.clearData) {
    ozbuilderd.prototype.clearData = function () {
        this.gProcWfProc = new OZwfProcess();
        this.gProcActs = new ClsCollJ();
        this.gProcLinks = new ClsCollJ();
    }
}
if (!ozbuilderd.prototype.doReadResource) {
    ozbuilderd.prototype.doReadResource = function (sResourcePathAddress) {
...
if (!ozbuilderd.prototype.InitDraw) {
    ozbuilderd.prototype.InitDraw = function (sDefContent) {
        this.clearData();
        // Resource
        ...
        if (this.showContent(sDefContent, true) == true) {
            //drawGoDiagram() //showContent내부에서 처리
...
if (!ozbuilderd.prototype.Init) {
    ozbuilderd.prototype.Init = function () {
        ...
        if (this.bInitCalled === false) {
            ...
            this.myDiagram.addDiagramListener("ObjectDoubleClicked", function (e) {
                ...
                if (e.subject.part instanceof go.Node) {
                    ...
                    refBuilder.gEvtArgs.Purpose = pWfAct.ActName; //D3FBuilder Only
                    //Web에서는 Frame이름으로??
        ...
        if (ozw_IsNull(this.myCMDiv) == false) {
            ...
            cxTool.showContextMenu = function (contextmenu, obj) {
                var diagram = this.diagram; // this ==> cxTool. not MyBuilder[ozbuilderd]
                ...
                if (refBuilder.Editable == false) {
                    this.stopTool(); return;
                }
                            ...
                            if (pWfAct.design_finish === true) {
	                            ...
	                            if (pWfAct.getConnectables(pCollCands) == true) {
	                                ...
	                                for (var i = 0; i < pCollCands.Count; i++) {
                                        ...
                                        sHTML += "<li><a href='#' id='" + sName + "' onclick='cxcommand2(event)'><span style='padding-right:3px'><img src='../img/wf/d3f/" + sImg.toLowerCase() + ".png' class='oz-wf-cm-d-img'/></span><span>" + sCap + " 추가" + "</span></a></li>";
	                                    iMenuCount += 1;
	                                }                                
                            	}
                            }
...
if (!ozbuilderd.prototype.CheckIn) {
    ozbuilderd.prototype.CheckIn = function () {
        if (this.validateContent() == true) return false;
...
if (!ozbuilderd.prototype.validateContent) {
    ozbuilderd.prototype.validateContent = function () {
...
if (!ozbuilderd.prototype.Save) {
    ozbuilderd.prototype.Save = function () {
        if (this.checkCircular() == true) return false;
        if (this.doSaveContent() == false) return false;
        if (this.callTM("save") == false) return false;
        return true;
...
if (!ozbuilderd.prototype.checkCircular) {
    ozbuilderd.prototype.checkCircular = function () {
...
if (!ozbuilderd.prototype.hasSameAct) {
    ozbuilderd.prototype.hasSameAct = function (refWfAct, refCollPP, sErr) {
...
if (!ozbuilderd.prototype.doSaveContent) {
    ozbuilderd.prototype.doSaveContent = function () {
        //save meta
        var i, j, con = "";
...
if (!ozbuilderd.prototype.showContent) {
        ozbuilderd.prototype.showContent = function (sContent, bInitDraw) {
...
if (!ozbuilderd.prototype.createActivities) {
    ozbuilderd.prototype.createActivities = function (xActs, bUpdate, bWIthUI) {
...
if (!ozbuilderd.prototype.createTransitions) {
    ozbuilderd.prototype.createTransitions = function (xLnks, bUpdate) {
        if (typeof (bUpdate) === "undefined") {
            bUpdate = false;
        }
...
if (!ozbuilderd.prototype.RefreshStatus) {
    ozbuilderd.prototype.RefreshStatus = function () {
            //==showExecInfo
...
if (!ozbuilderd.prototype.HandleStatusActions) {
    ozbuilderd.prototype.HandleStatusActions = function (sTaskType, sStatus, sActions) {
...
if (!ozbuilderd.prototype.HideStatusActions) {
    ozbuilderd.prototype.HideStatusActions = function (sActions) {
...
if (!ozbuilderd.prototype.SetProcessDataFieldValue) {
    ozbuilderd.prototype.SetProcessDataFieldValue = function (sName, sValue) {
        var pWfDF;
    ...
    if (!ozbuilderd.prototype.applyActData) {
        ozbuilderd.prototype.applyActData = function (xData, bUpdate) {
            //xData = ActivityData
    ...
    if (!ozbuilderd.prototype.initDiagram) {
        ozbuilderd.prototype.initDiagram = function (sParentID) {
            //Define UI template
            ...
            myD.validCycle = go.Diagram.CycleNotDirected; //D3FBuilder Only
            myD.undoManager.isEnabled = true; //잘못된 링크제거 가능하게 하기 위해(undo)
            return myD;
        }
    }
    if (!ozbuilderd.prototype.makePort) {
        ozbuilderd.prototype.makePort = function (isIn, isMonitor) {
            //dataFlow Sample
            var name = "Out";
            if (isIn) name = "In";
    ...
    if (!ozbuilderd.prototype.makeTemplate) {
        ozbuilderd.prototype.makeTemplate = function (typename, inports, outports) {
            var node = $go(go.Node, "Table",
                ...
                $go(go.Panel, "Auto",
                    ...
                    $go(go.Panel, "Table",
                        ...
                        $go(go.RowColumnDefinition, { row: 0, height: 8 }),
                        ...
                        $go(go.Shape, "Rectangle",
                            {
                                ...
                                height: 8, margin: new go.Margin(0, 1, 0, 0), stretch: go.GraphObject.Horizontal
                            },
                        ...
                        $go(go.TextBlock,
                            {
                                ...
                                stroke: "white", font: "9px Malgun Gothic, sans-serif", margin: new go.Margin(0, 4, 0, 0)
                            },
                        ...
                        $go(go.Picture,
                            {
                                ...
                                source: "../img/wf/d3f/error.png", opacity: 0,
                        ...
                        $go(go.Picture, {
                            source: "../img/wf/d3f/default.png", row: 1, column: 0,
                            width: 32, height: 32, margin: new go.Margin(0, 0, 0, 4)
                        },
                            new go.Binding("source", "imageid", function (v) { return "../img/wf/d3f/" + v.toLowerCase() + ".png"; })
                        ),
                        $go(go.TextBlock,
                            {
                                name: "CapLabel", row: 1, column: 1, alignment: go.Spot.Center,
                                font: "11px Malgun Gothic, sans-serif", maxSize: new go.Size(80, 36),
                            ...
                            new go.Binding("text", "caption"),
                            {
                                ...
                                $go(go.Adornment, "Auto",
                                    ...
                                    new go.Binding("visible", "caption", function (t) {
                                        //var node = myDiagram.findNodeForData(data);updateTargetBindings
                                        //return (node.findObject("caption").lineCount > 1 ? true : false);
                                        return (t.length > 16 ? true : false);
                                    }),

                                    $go(go.Shape, { fill: "#FFFFCC" }),
                                    $go(go.TextBlock, { margin: 4, font: "11px Malgun Gothic, sans-serif", stroke: "#365064" },
                                        new go.Binding("text", "caption"))
                                )  // end of Adornment
                            }
                        ),
                        $go(go.Picture,
                            {
                                ...
                                source: "../img/wf/d3f/completed.png", opacity: 0,
    ...
    if (!ozbuilderd.prototype.makeDiagram) {
        ozbuilderd.prototype.makeDiagram = function (myD, isMonitor) {
            //Define Node
            ...
            var node = $go(go.Node, "Table",
                ...
                $go(go.Picture, {
                        row: 0, column: 0, name: "BODY", source: "../img/wf/d3f/start.png",
                        width: 32, height: 32, margin: new go.Margin(0, 0, 0, 0)
                    }
                ),
                $go(go.Panel, "Horizontal",
                    { row: 1, column: 0 },
                    this.makePort(false, isMonitor)
                )
            );
            ...
            node = $go(go.Node, "Table",
                ...
                $go(go.RowColumnDefinition,
                    { row: 0, alignment: go.Spot.Top }),
                $go(go.Picture, {
                    row: 1, column: 0, name: "BODY", source: "../img/wf/d3f/end.png",
                    width: 32, height: 32, margin: new go.Margin(0, 0, 0, 0)
                    }
                ),
                $go(go.Panel, "Horizontal",
                    { row: 0, column: 0},
                    this.makePort(true, isMonitor)
                )
            );
            ...
            go.Diagram.inherit(d3fTreeLayout, go.TreeLayout);
            d3fTreeLayout.prototype.assignTreeVertexValues = function (v) {
                if (v.node.category != "endprocess") {
                    var outLinks = v.node.findLinksOutOf();
                    if (outLinks.count <= 1) {
                        v.layerSpacing = 50;
    ...
    if (!ozbuilderd.prototype.callTM) {
        ozbuilderd.prototype.callTM = function (sMethod) {
            var delim = "!%^";
    ...
    if (!ozbuilderd.prototype.callTM_NewDel) {
        ozbuilderd.prototype.callTM_NewDel = function (pWfAct, bDel) {
    ...
    if (!ozbuilderd.prototype.GetFromAppIDValueList) {
        ozbuilderd.prototype.GetFromAppIDValueList = function (pWfAct) {
    ...
    if (!ozbuilderd.prototype.GetInfoFromAct) {
        ozbuilderd.prototype.GetInfoFromAct = function (pWfAct, sInfoType) {
    ...
    if (!ozbuilderd.prototype.GetInfoToAct) {
        ozbuilderd.prototype.GetInfoToAct = function (pWfAct, sInfoType) {
...
if (!ozbuilderd.prototype.checkUniqueIdx) {
    ozbuilderd.prototype.checkUniqueIdx = function (sName) {
...
if (!ozbuilderd.prototype.getNewName) {
    ozbuilderd.prototype.getNewName = function (sCap) {
        this.gUniqueIdx += 1;
        return sCap + "_" + this.gUniqueIdx; //.toString;
    }
}

if (!ozbuilderd.prototype.tryLink) {
    ozbuilderd.prototype.tryLink = function (pWfActF, pWfActT) {
...
if (!ozbuilderd.prototype.doDeleteNode) {
    ozbuilderd.prototype.doDeleteNode = function (refObj) {
        var refBuilder = getCtl(getRefBuilderID());
        //do DeleteNode 
...
if (!ozbuilderd.prototype.doDeleteLink) {
    ozbuilderd.prototype.doDeleteLink = function (refObj) {
...
if (!ozbuilderd.prototype.InitServer) {
    ozbuilderd.prototype.InitServer = function (selector, refAgent) {
        ...
        if (sRsrcID == "_delete") {
            ...
            if (refObj instanceof go.Node) {
                if (refBuilder.ManualActivityDelete == true) {
                    ...
                    pWfAct = refObj.data;
                    refBuilder.gEvtArgs.Purpose = pWfAct.ActName; //D3FBuilder Only <-"BeforeActivityDelete";
                    refBuilder.gEvtArgs.Address = this.GetInfoFromAct(pWfAct, "activityname"); //getPreActs(pWfAct);
                    refBuilder.gEvtArgs.Content = this.GetInfoToAct(pWfAct, "activityname"); //getPostActs(pWfAct);
                    refBuilder.gEvtArgs.Extra = pWfAct.key;
