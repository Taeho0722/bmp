...
if(flag == "1"){actionUrl = "web_chart_campprogressinfo_sub01.jsp";}
...
function layerPopOpen(popupN){
  ...
  //레이어에 스크롤이 생기면 draggable 기능에 버그 발생 (일부 PC)
  //스크롤 클릭 후 마우스를 계속 따라다니는 현상
  //해당 현상을 방지하기 위해 처리 
  if(popupN == "layer_mdcalendar"){
	  $layer.draggable({
		  //handle:'.tit',//해당 영역만 이동 가능
		  cancel : "#list_div"//해당 영역만 제외하고 이동 가능
		});  
  }else{
	  $layer.draggable();
  }
...
function eventBrandList(){
  ...
  listTm.clear();
	...
	for(var i=0; i<listTmResult_json.length; i++) {
		...
		if(md_nm.length > 5){md_nm = md_nm.substring(0,3)+".."}
